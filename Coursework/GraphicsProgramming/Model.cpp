...
bool Model::loadModel(char* filename)
{
	...
	{
		int V = (faces[i]-1);
		vertex.push_back(verts[V].x);
		vertex.push_back(verts[V].y);
		vertex.push_back(verts[V].z);
	}
	for (int i = 2; i < faces.size(); i += 3)
	{
		int T = (faces[i]-1);
		normals.push_back(norms[T].x);
		normals.push_back(norms[T].y);
		normals.push_back(norms[T].z);
	}
	for (int i = 1; i < faces.size(); i += 3)
	{
		int P = (faces[i]-1);
		texCoords.push_back(texCs[P].x);
		texCoords.push_back(texCs[P].y);
